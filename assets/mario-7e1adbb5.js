import{S as s,P as c,W as p,V as N,g as X,M as P,a as T,j as g,o as w,C as v}from"./three.module-d899fbfe.js";import{B}from"./bufferShader-61810f9d.js";import{g as S,h as G,x as C,y as J,o as D,c as h}from"./index-0d4a0f96.js";const b=`// Overworld theme based on notes from geoffrey1218's famitracker cover\r
// Level clear theme based on notes from WheeljackDude's famitracker cover\r
\r
float SquareWave25( float f, float x )\r
{\r
    return floor( 4.0 * floor( f * x ) - floor( 4.0 * f * x ) + 1.0 );\r
}\r
\r
float SquareWave50( float f, float x )\r
{\r
    return floor( 2.0 * floor( f * x ) - floor( 2.0 * f * x ) + 1.0 );\r
}\r
\r
float SinWave( float f, float x )\r
{\r
    return sin( f * x * 2.0 * 3.14 );\r
}\r
\r
float SawtoothWave( float q, float x )\r
{\r
    float f = fract( x ) - q;\r
    f /= (f >= 0.0 ? 1.0 : 0.0) - q;\r
    return f * 2.0 - 1.0;\r
...
{\r
    return fract( sin( 123523.9898 * x ) * 43758.5453 );\r
}\r
\r
float Jump( float t )\r
{\r
    if ( t < 0.0 || t > 0.5 )\r
    {\r
        return 0.0;\r
    ...
    \r
\r
	float freq = mix( 250.0, 330.0, t / 0.5 );\r
    \r
    float waveSwitchT = 0.068;\r
    if ( t < waveSwitchT ) \r
    {\r
        return SquareWave50( freq, t ) * 0.5;\r
    }\r
    else\r
    {\r
        return SquareWave25( freq, t ) * clamp( 1.0 - ( t - 0.1 ) / ( 0.5 - 0.1 ), 0.0, 1.0 ) * 0.5;\r
    }\r
}\r
\r
float Stomp( float t )\r
{\r
    if ( t < 0.0 || t > 0.25 )\r
    {\r
        return 0.0;\r
 ...
 \r
    float freq = mix( 200.0, 300.0, t / 0.2 );\r
\r
    if ( t <= 0.1 )\r
    {\r
    	return SquareWave50( freq, t ) * clamp( t / 0.1, 0.0, 1.0 );\r
    }\r
    else\r
    {\r
        return SquareWave50( freq, t ) * clamp( ( t - 0.1 ) / 0.15, 0.0, 1.0 );\r
    }\r
}\r
\r
float PowerUp( float t )\r
{\r
    if ( t < 0.0 || t > 0.9 )\r
    {\r
        return 0.0;\r
    ...
    \r
    float freq = 250.0 + mod( t, 0.3 ) * 50.0;\r
    return mix( SquareWave50( freq, t ), SquareWave25( freq, t ), mod( t, 0.3 ) > 0.2 ? 0.5 : 0.0 ) * 0.5;\r
}\r
\r
float Bump( float t )\r
{\r
    if ( t < 0.0 || t > 0.15 )\r
    {\r
        return 0.0;\r
    ...
    \r
    float freq = mix( 150.0, 130.0, abs( t - 0.075 ) / 0.075 );\r
    return SquareWave50( freq, t ) * 0.5 + SinWave( freq, t ) * 0.5;\r
}\r
\r
float Coin( float t )\r
{\r
    if ( t < 0.0 || t > 0.75 )\r
    {\r
        return 0.0;\r
    ...
    \r
    float freq = t > 0.1 ? 600.0 : 480.0;\r
    return SquareWave50( freq, t ) * clamp( 1.0 - ( t - 0.1 ) / 0.65, 0.0, 1.0 );\r
}\r
\r
float DownTheFlagpole( float t )\r
{\r
    if ( t < 0.0 || t > 1.0 )\r
    {\r
        return 0.0;\r
    ...
    \r
    float freq = floor( mix( 200.0, 600.0, t ) * 0.1 ) * 10.0;\r
	return SquareWave50( freq, t ) * clamp( 1.0 - ( t - 0.95 ) / 0.05, 0.0, 1.0 );\r
}\r
\r
float InstrumentMain( float f, float t )\r
{\r
    float ret = 0.0;\r
    ret += SquareWave50( f, t ) * clamp( 1.0 - t / 0.250, 0.0, 1.0 ) * 0.5;\r
    return ret;\r
}\r
\r
float InstrumentBass( float f, float t )\r
{\r
	return SinWave( f, t ) * clamp( 1.0 - ( t - 0.1 ) / 0.01, 0.0, 1.0 );\r
}\r
\r
float InstrumentDrums( float f, float t )\r
{\r
    if ( f == 1.0 )\r
    {\r
        // open\r
    	return Noise( t ) * clamp( 1.0 - ( t - 0.08 ) / ( 0.001 ), 0.0, 1.0 ) * 0.66;  \r
    }\r
    else if ( f == 2.0 )\r
    {\r
        // close\r
        return Noise( t ) * clamp( 1.0 - ( t - 0.0165 ) / ( 0.001 ), 0.0, 1.0 ) * 0.66;\r
    }\r
    else\r
    {\r
    	// kick\r
        return SquareWave50( 100.0, t ) * clamp( 1.0 - ( t - 0.0165 ) / ( 0.001 ), 0.0, 1.0 );\r
    }\r
}\r
\r
// note frequencies\r
const float E2	=  82.41;\r
const float F2	=  87.31;\r
const float G2 	=  98.00;\r
const float GH2 = 103.83;\r
const float AH2 = 116.54;\r
const float C3 	= 130.81;\r
const float CH3	= 138.59;\r
const float D3 	= 146.83;\r
const float DH3 = 155.56;\r
const float E3 	= 164.81;\r
const float F3	= 174.61;\r
const float FH3 = 185.00;\r
const float G3 	= 196.00;\r
const float GH3 = 207.65;\r
const float A3 	= 220.00;\r
const float AH3	= 233.08;\r
const float B3 	= 246.94;\r
const float C4 	= 261.63;\r
const float D4  = 293.66;\r
const float DH4 = 311.13;\r
const float E4 	= 329.63;\r
const float F4 	= 349.23;\r
const float FH4 = 369.99;\r
const float G4 	= 392.00;\r
const float GH4 = 415.30;\r
const float A4	= 440.00;\r
const float AH4 = 466.16;\r
const float C5  = 523.25;\r
const float G5  = 783.99;\r
\r
#define N( off, freq ) 	if( pos > patternPos + float( off ) ) { notePos = patternPos + float( off ); noteFreq = float( freq ); }\r
#define PEND			patternPos += 32.0;\r
\r
const float TimeToPos = 1000.0 / 75.0;\r
const float PosToTime = 1.0 / TimeToPos;\r
\r
float OverworldMelody( float time )\r
{\r
    float pos			= time * TimeToPos;\r
    float noteFreq 		= 0.0;\r
    float notePos		= 0.0;\r
    float patternPos	= 0.0;\r
    \r
#define P0  N(0,E4)N(2,E4)N(6,E4)N(10,C4)N(12,E4)N(16,G4)PEND\r
#define P1  N(0,C4)N(6,G3)N(12,E3)N(18,A3)N(22,B3)N(26,AH3)N(28,A3)PEND\r
#define P2  N(0,G3)N(3,E4)N(6,G4)N(8,A4)N(12,F4)N(14,G4)N(18,E4)N(22,C4)N(24,D4)N(26,B3)PEND\r
#define P3  N(4,G4)N(6,FH4)N(8,F4)N(10,DH4)N(14,E4)N(18,GH3)N(20,A3)N(22,C4)N(26,A3)N(28,C4)N(30,D4)PEND\r
#define P4  N(4,G4)N(6,FH4)N(8,F4)N(10,DH4)N(14,E4)N(18,C5)N(22,C5)N(24,C5)PEND\r
#define P5  N(4,DH4)N(10,D4)N(16,C4)PEND\r
#define P6  N(0,C4)N(2,C4)N(6,C4)N(10,C4)N(12,D4)N(16,E4)N(18,C4)N(22,A3)N(24,G3)PEND\r
#define P7  N(0,C4)N(2,C4)N(6,C4)N(10,C4)N(12,D4)N(14,E4)PEND\r
    \r
    P0 P1 P2 P1 P2 P3 P4 P3 P5 P3 \r
    P4 P3 P5 P6 P7 P6 P0 P1 P2 P1\r
        \r
#undef P0\r
#undef P1\r
#undef P2\r
...
#undef P7\r
\r
	return InstrumentMain( noteFreq, ( pos - notePos ) * PosToTime );\r
}\r
\r
float OverworldHarmony( float time )\r
{\r
    float pos			= time * TimeToPos;\r
    float noteFreq 		= 0.0;\r
    float notePos		= 0.0;\r
    float patternPos	= 0.0;\r
    \r
#define P0  N(0,FH3)N(2,FH3)N(6,FH3)N(10,FH3)N(12,FH3)N(16,B3)N(24,G3)PEND\r
#define P1  N(0,E3)N(6,C3)N(12,G2)N(18,C3)N(22,D3)N(26,CH3)N(28,C3)PEND\r
#define P2  N(0,C3)N(3,G3)N(6,B3)N(8,C4)N(12,A3)N(14,B3)N(18,A3)N(22,E3)N(24,F3)N(26,D3)PEND    \r
#define P3  N(4,E4)N(6,DH4)N(8,D4)N(10,B3)N(14,C4)N(18,E3)N(20,F3)N(22,G3)N(26,C3)N(28,E3)N(30,F3)PEND    \r
#define P4  N(4,E4)N(6,DH4)N(8,D4)N(10,B3)N(14,C4)N(18,F4)N(22,F4)N(24,F4)PEND\r
#define P5  N(4,GH3)N(10,F3)N(16,E3)PEND\r
#define P6  N(0,GH3)N(2,GH3)N(6,GH3)N(10,GH3)N(12,AH3)N(16,G3)N(18,E3)N(22,E3)N(24,C3)PEND\r
#define P7  N(0,GH3)N(2,GH3)N(6,GH3)N(10,GH3)N(12,AH3)N(14,G3)PEND\r
    \r
    P0 P1 P2 P1 P2 P3 P4 P3 P5 P3 \r
    P4 P3 P5 P6 P7 P6 P0 P1 P2 P1\r
        \r
...
\r
\r
float OverworldBass( float time )\r
{\r
    float pos			= time * TimeToPos;\r
    float noteFreq 		= 0.0;\r
    float notePos		= 0.0;\r
    ...
    \r
\r
#define P0 N(0,D3)N(2,D3)N(6,D3)N(10,D3)N(12,D3)N(16,G4)N(24,G3)PEND\r
#define P1 N(0,G3)N(6,E3)N(12,C3)N(18,F3)N(22,G3)N(26,FH3)N(28,F3)PEND\r
#define P2 N(0,E3)N(3,C4)N(6,E4)N(8,F4)N(12,D4)N(14,E4)N(18,C4)N(22,A3)N(24,B3)N(26,G3)PEND    \r
#define P3 N(0,C3)N(6,G3)N(12,C4)N(16,F3)N(22,C4)N(24,C4)N(28,F3)PEND\r
#define P4 N(0,C3)N(6,E3)N(12,G3)N(14,C4)N(18,G5)N(22,G5)N(24,G5)N(28,G3)PEND\r
#define P5 N(0,C3)N(4,GH3)N(10,B3)N(16,C4)N(16,C4)N(22,G3)N(24,G3)N(28,C3)PEND\r
#define P6 N(0,GH2)N(6,DH3)N(12,GH3)N(16,G3)N(22,C3)N(28,G2)PEND\r
    \r
    P0 P1 P2 P1 P2 P3 P4 P3 P5 P3 \r
    P4 P3 P5 P6 P6 P6 P0 P1 P2 P1\r
        \r
#undef P0\r
#undef P1\r
#undef P2\r
...
#undef P6        \r
\r
    return InstrumentBass( noteFreq, ( pos - notePos ) * PosToTime );\r
}\r
\r
float OverworldDrums( float time )\r
{\r
    float pos			= time * TimeToPos;\r
    float noteFreq 		= 0.0;\r
    float notePos		= 0.0;\r
    ...
    // 2 - close\r
    // 3 - kick\r
#define P0 N(0,1)N(4,2)N(6,1)N(10,2)N(12,1)N(16,1)N(24,1)N(26,2)N(28,2)N(30,2)PEND\r
#define P1 N(0,3)N(4,2)N(7,2)N(8,1)N(12,2)N(15,2)N(16,3)N(20,2)N(23,2)N(24,1)N(28,2)N(31,2)PEND\r
#define P2 N(0,1)N(4,2)N(6,1)N(10,2)N(12,1)N(15,1)N(22,1)N(26,2)N(28,2)N(30,2)PEND    \r
    \r
    P0 P1 P1 P1 P1 P1 P1 P1 P1 P1\r
    P1 P1 P1 P2 P2 P2 P0 P1 P1 P1\r
        \r
#undef P0\r
#undef P1        \r
#undef P2\r
    \r
    return InstrumentDrums( noteFreq, ( pos - notePos ) * PosToTime );\r
}\r
\r
const float TimeToPos2 = 0.5 * TimeToPos;\r
const float PosToTime2 = 1.0 / TimeToPos2;\r
\r
float LevelClearMelody( float time )\r
{\r
    float pos			= time * TimeToPos2;\r
    float noteFreq 		= 0.0;\r
    float notePos		= 0.0;\r
    float patternPos	= 0.0;\r
    \r
	N(0,G2)N(1,C3)N(2,E3)N(3,G3)N(4,C4)N(5,E4)N(6,G4)N(9,E4)N(12,GH2)N(13,C3)N(14,DH3)N(15,GH3)N(16,C4)N(17,DH4)N(18,GH4)N(21,DH4)N(24,AH2)N(25,DH3)N(26,F3)N(27,AH3)N(28,D4)N(29,F4)N(30,AH4)N(33,AH4)N(34,AH4)N(35,AH4)N(36,E4)\r
\r
    return InstrumentMain( noteFreq, ( pos - notePos ) * PosToTime2 );\r
}\r
\r
float LevelClearHarmony( float time )\r
{\r
    float pos			= time * TimeToPos2;\r
    float noteFreq 		= 0.0;\r
    float notePos		= 0.0;\r
    float patternPos	= 0.0;\r
    \r
	N(1,E2)N(2,G2)N(3,C3)N(4,E3)N(5,G3)N(6,C4)N(9,G3)N(13,E2)N(14,GH2)N(15,C3)N(16,DH3)N(17,GH3)N(18,C4)N(21,GH3)N(25,F2)N(26,AH2)N(27,D3)N(28,F3)N(29,AH3)N(30,D4)N(33,D4)N(36,C5)\r
\r
    return InstrumentMain( noteFreq, ( pos - notePos ) * PosToTime2 );\r
}\r
\r
float LevelClearBass( float time )\r
{\r
    float pos			= time * TimeToPos2;\r
    float noteFreq 		= 0.0;\r
    float notePos		= 0.0;\r
    float patternPos	= 0.0;\r
    \r
	N(3,C3)N(4,E3)N(5,G3)N(6,E4)N(9,C4)N(15,C3)N(16,DH3)N(17,GH3)N(18,DH4)N(21,C4)N(27,D3)N(28,F3)N(29,AH3)N(30,F4)N(33,D4)N(34,D4)N(35,D4)N(36,C4)\r
\r
    return InstrumentBass( noteFreq, ( pos - notePos ) * PosToTime2 );\r
}\r
\r
float GameSounds( float time )\r
{\r
    // play sounds a bit earlier\r
    time += 0.1;\r
\r
    float ret = 0.0;\r
    \r
    float marioBigJump1 = 27.1;\r
    float marioBigJump2 = 29.75;\r
    float marioBigJump3 = 35.05;    \r
    \r
    \r
    // Jump sounds\r
    float jumpTime = time - 38.7;\r
    if ( jumpTime <= 0.0 ) { jumpTime = time - marioBigJump3 - 1.2 - 0.75; }    \r
    if ( jumpTime <= 0.0 ) { jumpTime = time - marioBigJump3 - 1.2; }\r
    if ( jumpTime <= 0.0 ) { jumpTime = time - marioBigJump3; }\r
    if ( jumpTime <= 0.0 ) { jumpTime = time - 34.15; }    \r
    if ( jumpTime <= 0.0 ) { jumpTime = time - 33.7; }\r
    if ( jumpTime <= 0.0 ) { jumpTime = time - 32.3; }    \r
    if ( jumpTime <= 0.0 ) { jumpTime = time - marioBigJump2 - 1.0; }    \r
    if ( jumpTime <= 0.0 ) { jumpTime = time - marioBigJump2; }    \r
    if ( jumpTime <= 0.0 ) { jumpTime = time - marioBigJump1 - 1.0; }    \r
    if ( jumpTime <= 0.0 ) { jumpTime = time - marioBigJump1; }    \r
    if ( jumpTime <= 0.0 ) { jumpTime = time - 25.75; }    \r
	if ( jumpTime <= 0.0 ) { jumpTime = time - 24.7; }        \r
    if ( jumpTime <= 0.0 ) { jumpTime = time - 23.0; } \r
    if ( jumpTime <= 0.0 ) { jumpTime = time - 21.7; }    \r
    if ( jumpTime <= 0.0 ) { jumpTime = time - 19.65; }   \r
    if ( jumpTime <= 0.0 ) { jumpTime = time - 18.7; }    \r
    if ( jumpTime <= 0.0 ) { jumpTime = time - 15.1; } \r
    if ( jumpTime <= 0.0 ) { jumpTime = time - 13.62; }    \r
    if ( jumpTime <= 0.0 ) { jumpTime = time - 11.05; }\r
    if ( jumpTime <= 0.0 ) { jumpTime = time - 9.0; }\r
    if ( jumpTime <= 0.0 ) { jumpTime = time - 7.8; }\r
    if ( jumpTime <= 0.0 ) { jumpTime = time - 6.05; }\r
    if ( jumpTime <= 0.0 ) { jumpTime = time - 5.0; }\r
    if ( jumpTime <= 0.0 ) { jumpTime = time - 4.2; }\r
	ret += Jump( jumpTime );                           \r
\r
    // block bump sounds\r
    float bumpTime = time - 33.9;\r
    if ( bumpTime <= 0.0 ) { bumpTime = time - 22.4; }\r
    if ( bumpTime <= 0.0 ) { bumpTime = time - 15.4; }\r
    if ( bumpTime <= 0.0 ) { bumpTime = time - 5.3; }\r
    ret += Bump( bumpTime );\r
    \r
    // coin sounds\r
    float coinTime = time - 33.9;\r
    if ( coinTime <= 0.0 ) { coinTime = time - 22.4; }\r
    if ( coinTime <= 0.0 ) { coinTime = time - 5.4; }    \r
    ret += Coin( coinTime );    \r
\r
    float stompTime = time - 26.3;\r
    if ( stompTime <= 0.0 ) { stompTime = time - 25.3; }\r
    if ( stompTime <= 0.0 ) { stompTime = time - 23.5; }    \r
    if ( stompTime <= 0.0 ) { stompTime = time - 20.29; }    \r
    if ( stompTime <= 0.0 ) { stompTime = time - 10.3; }    \r
    ret += Stomp( stompTime );\r
    \r
	ret += PowerUp( time - 17.0 );    \r
\r
    ret += DownTheFlagpole( time - 38.95 );    \r
    \r
    return ret;\r
}\r
\r
vec2 mainSound( in int samp, float time )\r
{    \r
    float ret = 0.0;\r
    \r
    float overworldTime  = max( time -  1.0, 0.0 );\r
    float levelClearTime = max( time - 40.2, 0.0 );\r
\r
    ret += OverworldMelody( overworldTime ) 	* 0.3;\r
    ret += OverworldHarmony( overworldTime ) 	* 0.2;\r
    ret += OverworldBass( overworldTime )   	* 0.2;\r
    ret += OverworldDrums( overworldTime )		* 0.15;  \r
    \r
    // overworld theme fadout before the castle\r
    ret *= 1.0 - smoothstep( 38.0, 40.0, time );\r
    \r
    ret += LevelClearMelody( levelClearTime ) 	* 0.3;\r
    ret += LevelClearHarmony( levelClearTime ) 	* 0.3;\r
    ret += LevelClearBass( levelClearTime ) 	* 0.3;\r
    \r
    ret += GameSounds( time )					* 0.2;\r
    \r
    // disable output on first frames\r
    ret = time <= 1.0 ? 0.0 : ret;\r
    \r
    return vec2( ret, ret );\r
}`,E={id:"shader"},F=S({__name:"mario",setup(Y){let a=G(null);return C(()=>{const x=document.getElementById("shader"),r=new s,i={width:x.offsetWidth,height:x.offsetHeight};window.addEventListener("resize",()=>{i.width=window.innerWidth,i.height=window.innerHeight,e.aspect=i.width/i.height,e.updateProjectionMatrix(),o.setSize(i.width,i.height),o.setPixelRatio(Math.min(window.devicePixelRatio,2))});const e=new c(75,i.width/i.height,.1,100);e.position.set(0,0,.587),r.add(e);const o=new p({antialias:!0});o.setSize(i.width,i.height),o.setPixelRatio(Math.min(window.devicePixelRatio,2)),x==null||x.appendChild(o.domElement);let d;{new w;const{width:t,height:f}=o.getSize(new N),l=new X(t,f,o.pixelRatio),y=`
      uniform float iTime;
...
void main( )
{
    ...
    `;d=new B(o,e,y,{iResolution:{value:l},iTime:{value:0}},l.x,l.y);const u=new P(new T(1.6,.9),new g({map:d.readBuffer.texture}));r.add(u)}const n=new v,m=()=>{if(d){const t=n.getDelta();d.uniforms.iTime.value+=t,d.render()}o.render(r,e),a.value=window.requestAnimationFrame(m)};m()}),J(()=>{console.log(11111111111),cancelAnimationFrame(a.value)}),(x,r)=>(D(),h("div",E))}});export{F as default};
